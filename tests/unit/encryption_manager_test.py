...
class TestEncryptionManager(unittest.TestCase):
    def setUp(self):
        ...
        self.encryption_manager = EncryptionManager(encryption_status=True, config_manager=self.mock_config_manager)

    @patch('encryption_manager.urandom')  # Assuming urandom is in the same module, adjust the import path as necessary
    @patch('encryption_manager.time.time', side_effect=[100, 200])  # Mock time to simulate passage of time
    ...
    @patch('encryption_manager.time.time', side_effect=[100, 101])  # Mock time to ensure cache is used
    ...
    @patch('encryption_manager.b64decode')
    ...
    @patch('encryption_manager.b64encode')
    @patch('encryption_manager.time.time', return_value=1000)
    ...
    @patch('encryption_manager.Cipher')
